...
function ActiveCampaigns() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (!ignore && response.status == 200) {
        const sortedCampaigns = sortCampaigns(response.data);
  ...
  const sortCampaigns = (campaigns) => {
    const active = [];
    const success = [];
    const abortedOrExpired = [];

    campaigns.forEach((campaign) => {
      if (campaign.status === "active") {
        active.push(campaign);
      } else if (campaign.status === "success") {
        success.push(campaign);
      } else {
        abortedOrExpired.push(campaign);
      }
    });

    return [...active, ...success, ...abortedOrExpired];
  };
